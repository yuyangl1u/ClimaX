...
class RegionalForecastModule(LightningModule):
    ...
    def __init__(
            ...
            eta_min: float = 1e-8,
            lambda_advection: float = 10,  # 新增平流约束权重
            lambda_stability: float = 10,  # 新增垂直梯度约束权重
    ):
        super().__init__()
    ...
    def training_step(self, batch: Any, batch_idx: int):
        ...
        # ---------------------- 添加物理约束 ----------------------
        # 提取输入变量中的10m风速
        u_10m = x[:, variables.index("10m_u_component_of_wind"), ...]
        v_10m = x[:, variables.index("10m_v_component_of_wind"), ...]

        # 提取预测的2m温度（假设输出变量中包含 "2m_temperature"）
        T_pred = preds[:, out_variables.index("2m_temperature"), ...]

        # 获取网格间距（需从 region_info 中提取）
        delta_lon = region_info['delta_lon']  # 经度方向间距（单位：米）
        delta_lat = region_info['delta_lat']  # 纬度方向间距（单位：米）

        # 计算温度平流项 ∇T·u
        dTdx = torch.gradient(T_pred, dim=3)[0] / delta_lon.unsqueeze(0).unsqueeze(-1)  # 经度梯度
        dTdy = torch.gradient(T_pred, dim=2)[0] / delta_lat.unsqueeze(0).unsqueeze(-1)  # 纬度梯度
        advection = u_10m * dTdx + v_10m * dTdy
        advection_loss = torch.mean(advection ** 2)

        # 垂直梯度约束（假设输入变量包含土壤温度 "soil_temperature_0-7cm"）
        T_soil = x[:, variables.index("soil_temperature_0-7cm"), ...]
        dTdz = (T_pred - T_soil) / 1.93  # Δz = 2m - 0.07m = 1.93m
        stability_loss = torch.mean(dTdz ** 2)

        # 总损失 = 原损失 + 平流约束 + 垂直梯度约束
        total_loss = loss_dict["loss"] + \
                     self.hparams.lambda_advection * advection_loss + \
                     self.hparams.lambda_stability * stability_loss

        # 记录物理损失
        self.log("train/advection_loss", advection_loss, on_step=True, prog_bar=True)
        self.log("train/stability_loss", stability_loss, on_step=True, prog_bar=True)
        # ----------------------------------------------------------
