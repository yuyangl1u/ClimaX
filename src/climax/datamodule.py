...
class RegionalForecastDataModule(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        # 计算经纬度方向的实际间距（假设网格分辨率为5.625度）
        R = 6371e3  # 地球半径（米）
        delta_lon_degree = 5.625  # 经度网格间距（根据实际数据调整）
        delta_lat_degree = 5.625  # 纬度网格间距

        # 转换为米（纬度方向间距固定，经度方向随纬度变化）
        region_info['delta_lat'] = R * torch.deg2rad(torch.tensor(delta_lat_degree))
        region_info['delta_lon'] = R * torch.deg2rad(torch.tensor(delta_lon_degree)) * torch.cos(torch.deg2rad(lat))
